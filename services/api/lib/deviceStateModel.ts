...
export interface DeviceStateDocument {
  ...
  deviceId:  string;
  /** ObjectId of the user who owns the device / triggered the command. */
  userId:    ObjectId;
...
export async function insertDeviceState(
  ...
  // Convert the string userId from the JWT into an ObjectId.
  // This keeps the DB schema consistent with the `users` collection where
  // `_id` is an ObjectId, enabling efficient `$lookup` joins in aggregations.
